use crate::bookir::nav::NavTree;
use crate::config::{Config, OutputFile, OutputType};
...
use crate::bookir::{self, Book, ExtraItem};

use super::{Output, helpers};
...
pub fn gen_collected_output<C: Config + for<'a> serde::Deserialize<'a>>(
    ...
    let config: C = ctx
        .config
        .get(&format!("output.{output_name}"))
        .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?
        .unwrap_or_default();

    let mut chapter_list = IndexMap::new();
